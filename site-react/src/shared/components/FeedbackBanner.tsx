...
const FeedbackBanner: React.FC<FeedbackBannerProps> = ({ geneSymbol }) => {
  ...
  let url = config.contactUrl;

  if (geneSymbol) {
    url = `${config.contactPrefillUrl}&entry.1624035027=${geneSymbol}&entry.15683129=${geneSymbol}&entry.168426483=${geneSymbol}&entry.391072423=${geneSymbol}`;
  }
  return (
    <div className="my-6 flex items-start gap-2 rounded-lg border-l-4 border-amber-500 bg-amber-50 p-4 text-lg">
      ...
      <p className="text-gray-700">
        ...
        <a
          ...
          onClick={() => handleExternalLinkClick(config.contactUrl)}
        >
